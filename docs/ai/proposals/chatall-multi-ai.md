> **Proposal, not implemented.** Moved from the repo-root `AI_Design_Doc.md` (which confusingly shared its filename with the unrelated `docs/AI_Design_Doc.md`, now archived at `../design-history/original-rubric-design-draft.md`). This describes a side-by-side multi-model chat comparison feature (à la ChatALL). No `/api/multi-ai-chat` endpoint, router/microservice layer, or `prompts`/`responses`/`preferences` tables exist in the codebase today. Current AI chat (`/ai_chat`) talks to one provider at a time, per the assignment's configured provider — see `../architecture.md`.

# ChatALL Integration into CodeAssist

This document explains how the **Multi-AI Chat (ChatALL)** feature would extend the existing CodeAssist system, if built.
...
Reference: https://github.com/ai-shifu/ChatALL
