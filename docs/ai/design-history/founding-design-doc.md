> **Historical — whole-product founding doc, kept here for AI context.** Moved from `docs/CodeAssist_Design_Doc.md`. This predates the current implementation and covers the *entire* original CodeAssist proposal (auth, courses, submissions, DB schema), not just AI — most of the schema below (Student/Instructor tables, etc.) is superseded by the real models in `backend/api/models.py`. It's kept in the AI docs folder specifically because AI-powered feedback and the "Adaptive Learning Loop" (tracking recurring student mistakes across submissions) were core motivating goals from day one — this is the original source of the metacognition/history research questions behind what's now tracked as "Future work 2: student history / AI memory" (see `../README.md`). The "Create Rubric" / rubric grading direction mentioned here also never shipped — same story as `original-rubric-design-draft.md`.
...
*(These research questions are the direct ancestor of the current "coding_insights" mechanism and the open "student history / AI memory" work — see `../architecture.md` and `../README.md` for how thin that implementation currently is relative to this original ambition.)*
...
*(No `description` column was ever added to the real `Assignment` model — see `../README.md` gap #1.)*
...
----------
# Future Work (original list, 2022)
...
Most of these have since shipped (session auth, TA roles, Canvas integration). The last item — learning from common mistakes to suggest fixes for future students — is essentially the still-open "student history / AI memory" gap.
