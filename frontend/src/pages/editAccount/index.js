  ...
  const fetchData = async (id) => {
    try {
      const response = await fetch(
        `${process.env.REACT_APP_API_URL}/get_user_by_id?id=${id}`
      );
      if (!response.ok) {
      ...
      <Space direction="vertical" style={{ width: "100%" }}>
        ...
        >
          ...
          <Form.Item label="EID" name="sis_user_id">
            <Input
              placeholder={placeholders.sis_user_id || "Enter EID"}
              disabled
            />
          </Form.Item>
          ...
          <Form.Item label="EMAIL ADDRESS" name="email_address">
            <Input
              placeholder={placeholders.email_address || "Enter Email Address"}
              disabled
            />
          </Form.Item>
