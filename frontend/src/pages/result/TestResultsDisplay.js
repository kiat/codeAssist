...
const TestResultsDisplay = ({ viewMode, studentId, assignmentName, studentName, score, totalPoints, assignmentId, data, aiFeedbackEnabled, isModal = false, submissionId: submissionIdFromProps, onCancel }) => {
  ...
  const aiFeedbackStartTime = useRef(Date.now());
  const AI_FEEDBACK_TIMEOUT_MS = 60000; // 60 seconds
  ...
  const getAiAnnotations = useCallback(() => {
    ...
    if (data && data.ai_feedback !== undefined && data.ai_feedback !== null) {
      ...
      // Issue #345: Check if we've exceeded the timeout
      // This prevents infinite spinning when no API key is configured
      if (Date.now() - aiFeedbackStartTime.current > AI_FEEDBACK_TIMEOUT_MS) {
        console.log("AI Feedback timeout reached - feedback may not be available (check API key configuration).");
      ...
      <div style={{ position: "relative", maxHeight: "400px", overflowY: "scroll", border: "1px solid #ccc", padding: "10px" }}>
        ...
        {aiFeedbackEnabled && (
          <div style={{ position: "absolute", top: 5, right: 10, display: "flex", alignItems: "center", gap: "8px" }}>
            ...
            {/* Issue #345: Show timeout status instead of infinite spinning */}
            {loadingStatus === "timeout" && (
              <Tooltip title="AI feedback could not be generated. Check if an API key is configured for this course.">
                <Tag color="orange">AI Feedback Unavailable</Tag>
              </Tooltip>
            )}
          </div>
        )}
  ...
  const renderAiFeedbackSummary = () => {
    ...
    // Issue #345: Handle timeout state - show a warning instead of infinite spinning
    if (loadingStatus === "timeout") {
      return (
        <Alert
          ...
          description="AI feedback could not be generated. This may be because no API key is configured for this course. Please contact your instructor."
        />
      );
    }
