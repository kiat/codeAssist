...
export default function StudentInfoPanel({ assignmentName, studentName, score, totalPoints, active }) {
  ...
  const handleRequestSubmission = async () => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.REACT_APP_API_URL}/send_regrade_request`, {
        method: "POST",
        headers: {
  ...
  const handleGradeSubmission = async () => {
    ...
    try {
      const response = await fetch(`${process.env.REACT_APP_API_URL}/update_grade`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      {active && (
        <Space>
          ...
          <Space direction="vertical" size="middle">
            ...
            {Justification && (
              <div ref={justificationRef} tabIndex={-1} style={justificationStyle}>
          ...
          <Modal title="Regrade Request" open={RequestModalVisible} onCancel={() => setRequestModalVisible(false)} onOk={() => handleRequestSubmission()}>
            <Input.TextArea
              rows={4}
          ...
          <Modal title="Edit Grades" open={EditGradeModalVisible} onCancel={() => setEditGradeModalVisible(false)} onOk={() => handleGradeSubmission()}>
            <Input type="number" placeholder="Enter New Grade" onChange={(e) => setGrade(e.target.value)} />
          </Modal>
        </Space>
      )}
