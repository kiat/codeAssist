...
def test_create_extension_existing(client, mocker):
    with client.application.app_context():  # Ensure we are in the correct app context
        # Mock query to return an existing extension
        mock_query = mocker.patch("routes.assignment.db.session.query")
        ...
        # Assertions
        assert resp.status_code == 200
        mock_delete.assert_called_once()  # Ensure deletion is called for existing extension
        mock_commit.assert_called()  # Ensure commit was performed
