...
def create_user():
    ...
    db.session.add(People(**user_data))
    db.session.commit()
    res = db.session.query(People).filter_by(id=user_id)
    res = PeopleSchema().dump(res, many=True)[0]
...
def user_login():
    ...
    res = db.session.query(People).filter_by(email_address=email, password=password)
    res = PeopleSchema().dump(res, many=True)
...
def get_user():
    ...
    res = db.session.query(People).filter_by(email_address=email)
    res = PeopleSchema().dump(res, many=True)
    # print(res)
...
def get_user_by_id():
    ...
    person = db.session.query(People).filter_by(sis_user_id=insid)
    person = PeopleSchema().dump(person, many=True)[0]
